...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NBcoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "nbcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "nbcoind.pid"));
